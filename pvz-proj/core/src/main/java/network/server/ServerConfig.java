...
public final class ServerConfig {
    ...
    public ServerConfig(String bindHost, int port, int backlog) {
        ...
        if (port < 0 || port > 65_535) throw new IllegalArgumentException("server port must be between 0 and 65535");
        this.port = port;
    ...
    public static ServerConfig fromEnvironment() {
        ...
        if (host == null || host.isBlank()) host = System.getenv("PVZ_SERVER_BIND");
        ...
        try {
            if (portText != null && !portText.isBlank()) port = Integer.parseInt(portText.trim());
